<template>
  ...
  <div class="min-h-screen flex items-center justify-center px-6 py-12">
      ...
      <div class="rounded-sm border px-8 py-10 text-center" style="background:var(--card);border-color:var(--rule);">
        ...
        <form @submit.prevent="login" class="flex flex-col gap-4">
          <input v-model="password" type="password" placeholder="管理密码"
            ...
            @blur="e => e.target.style.borderColor='var(--rule)'" />
          <p v-if="loginError" class="text-xs" style="color:#c0392b;">{{ loginError }}</p>
          <button type="submit"
            class="w-full py-2.5 rounded-sm text-sm tracking-widest transition-all border"
            ...
            @mouseleave="e => { e.target.style.background='var(--tag-bg)'; e.target.style.color='var(--ink-light)'; e.target.style.borderColor='var(--rule)'; }">
            进入
          </button>
        </form>
      </div>
    ...
    <div v-else class="w-full max-w-[720px]">
      ...
      <section class="rounded-sm border px-6 py-6 mb-4" style="background:var(--card);border-color:var(--rule);">
        ...
        <div class="grid gap-4">
          <div class="flex items-center gap-3">
            ...
            <input v-model="profileForm.avatar" placeholder="图床链接 或 Emoji"
              class="flex-1 px-3 py-2 border rounded-sm text-sm focus:outline-none transition-colors"
              style="background:var(--paper);border-color:var(--rule);color:var(--ink);"
              @focus="e => e.target.style.borderColor='var(--accent)'" @blur="e => e.target.style.borderColor='var(--rule)'" />
          </div>
          <div class="flex items-center gap-3">
            <label class="text-xs w-16 shrink-0 tracking-wider" style="color:var(--ink-light);">姓名</label>
          ...
          <div class="flex items-start gap-3">
            <label class="text-xs w-16 shrink-0 pt-2 tracking-wider" style="color:var(--ink-light);">简介</label>
            <textarea v-model="profileForm.bio" rows="2"
              class="flex-1 px-3 py-2 border rounded-sm text-sm focus:outline-none resize-none transition-colors"
              style="background:var(--paper);border-color:var(--rule);color:var(--ink);"
              @focus="e => e.target.style.borderColor='var(--accent)'" @blur="e => e.target.style.borderColor='var(--rule)'"></textarea>
          </div>
      ...
      <section class="rounded-sm border px-6 py-6 mb-4" style="background:var(--card);border-color:var(--rule);">
        ...
        <div class="grid gap-4">
          <div class="flex items-center gap-3">
            ...
            <input v-model="settingsForm.favicon_url" placeholder="https://..."
              class="flex-1 px-3 py-2 border rounded-sm text-sm focus:outline-none transition-colors"
              style="background:var(--paper);border-color:var(--rule);color:var(--ink);"
              @focus="e => e.target.style.borderColor='var(--accent)'" @blur="e => e.target.style.borderColor='var(--rule)'" />
          </div>
          <div class="flex items-center gap-3">
            <label class="text-xs w-16 shrink-0 tracking-wider" style="color:var(--ink-light);">ICP</label>
            <input v-model="settingsForm.icp_number" placeholder="京ICP备..."
              class="flex-1 px-3 py-2 border rounded-sm text-sm focus:outline-none transition-colors"
              style="background:var(--paper);border-color:var(--rule);color:var(--ink);"
              @focus="e => e.target.style.borderColor='var(--accent)'" @blur="e => e.target.style.borderColor='var(--rule)'" />
          </div>
          <div class="flex items-center gap-3">
            <label class="text-xs w-16 shrink-0 tracking-wider" style="color:var(--ink-light);">公安</label>
            <input v-model="settingsForm.police_number" placeholder="京公网安备..."
              class="flex-1 px-3 py-2 border rounded-sm text-sm focus:outline-none transition-colors"
              style="background:var(--paper);border-color:var(--rule);color:var(--ink);"
              @focus="e => e.target.style.borderColor='var(--accent)'" @blur="e => e.target.style.borderColor='var(--rule)'" />
          </div>
      ...
      <p class="text-center mt-8 text-xs">
        <a href="/" class="tracking-wider hover:underline" style="color:var(--ink-faint);">← 返回主页</a>
      </p>
    </div>
  </div>
</template>
...
async function loadData() {
  try {
    ...
    if (sRes.ok) { const s = await sRes.json(); Object.assign(settingsForm, s); if (s.favicon_url) setFavicon(s.favicon_url) }
